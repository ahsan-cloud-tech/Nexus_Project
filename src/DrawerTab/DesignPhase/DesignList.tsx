...
export default function DesignList() {
      ...
      try {
        if (Platform.OS === 'android') {
          ...
          if (
            ...
            Alert.alert('Permission denied', 'Camera & Audio access is required.');
            return;
          }
        }
        ...
        if (cameraPermission !== 'granted' || microphonePermission !== 'granted') {
          Alert.alert('Permission denied', 'Please enable Camera & Mic in settings.');
        }
      } catch (error) {
  ...
  const handleImageClick = (imageData: { uri: string; fileName: string } | null, capturedPhoto: string | null) => {
  ...
  const handleModalSubmit = () => {
    ...
    if (
      ...
      Alert.alert('Warning', 'Please fill at least one field before submitting.');
      return;
    }
            ...
            onPress: async () => {
              try {
                ...
                const downloadData = {
                  ...
                  totalAttachments: formData.images.length + (formData.capturedPhoto ? 1 : 0)
                };
                ...
                setTimeout(() => {
                  Alert.alert(
                    ...
                    `• Comments: ${downloadData.comments ? `${downloadData.comments.length} characters` : 'No'}\n` +
                    `• Links: ${downloadData.links ? downloadData.links.split(',').length : 0}\n` +
                    `• Images: ${downloadData.imagesCount}\n` +
                    `• Captured Photos: ${downloadData.hasCapturedPhoto ? 'Yes' : 'No'}\n` +
          ...
          <View style={{ flex: 1 }}>
            ...
            {hasSubmittedData && (
              <View style={styles.submittedBadge}>
                <Ionicons name="checkmark-circle" size={hp('1.8%')} color="#1d9b20" />
    ...
    <Modal visible={isCameraVisible} animationType="slide">
      <View style={{ flex: 1, backgroundColor: 'black' }}>
        {device ? (
          ...
          <Text style={{ color: 'white', textAlign: 'center', marginTop: hp('6%') }}>
            Loading Camera...
          </Text>
        )}
    ...
    <Modal visible={isDrawingVisible} animationType="slide">
      <View style={{ flex: 1, backgroundColor: '#000' }}>
        ...
        <View style={styles.drawingBottomControls}>
          <TouchableOpacity style={styles.drawingBottomBtn} onPress={handleRetakePhoto}>
          ...
          <TouchableOpacity style={styles.drawingBottomBtn} onPress={handleAddText}>
            <Ionicons
              name="document-text"
              size={hp('3.5%')}
                  ...
                  {capturedPhotoPath || selectedImages.length > 0 ? (
                    <View style={styles.imagesContainer}>
                      {capturedPhotoPath && (
                        <TouchableOpacity 
                          ...
                          onPress={() => handleImageClick(null, capturedPhotoPath)}
                        >
                          <Ionicons name="camera" size={hp('3%')} color="#1d9b20" />
                          <Text style={styles.fileNameText} numberOfLines={1}>
                            [ Captured Photo - {formatTimestamp(new Date(photoTimestamp || Date.now()))} ]
                          </Text>
                          <TouchableOpacity onPress={removeCapturedPhoto}>
                            <Ionicons name="trash-outline" size={hp('2.4%')} color="red" />
                          </TouchableOpacity>
                        </TouchableOpacity>
                      )}
                      {selectedImages.map((image, index) => (
                        ...
                        >
                          <Ionicons name="image" size={hp('3%')} color="#1d9b20" />
                          <Text style={styles.fileNameText} numberOfLines={1}>
                            [ {image.fileName} ]
                          </Text>
                    ...
                    <View style={styles.uploadBox}>
                      <View style={styles.uploadIconsContainer}>
                        ...
                        >
                          <Ionicons name="camera" size={hp('5%')} color="#808080" />
                        ...
                        >
                          <Ionicons name="document" size={hp('5%')} color="#808080" />
          ...
          >
            <View style={styles.detailModalContent}>
              ...
              >
                ...
                {/* Images Section */}
                {(selectedFormData?.capturedPhoto || selectedFormData?.images.length > 0) && (
                  <View style={styles.formGroup}>
                    ...
                    {selectedFormData?.capturedPhoto && (
                      <TouchableOpacity 
                        ...
                        onPress={() => handleImageClick(null, selectedFormData.capturedPhoto!)}
                      >
                        <View style={styles.imageInfo}>
                          ...
                          <View style={styles.imageDetails}>
                            ...
                            <Text style={styles.imageTimestamp}>
                              {selectedFormData.photoTimestamp || 'No timestamp'}
                            </Text>
                          </View>
                        </View>
                        <View style={styles.imageActions}>
                          <TouchableOpacity 
                            ...
                            onPress={() => handleViewImage(selectedFormData.capturedPhoto!)}
                          >
                            <Ionicons name="download" size={hp('2.2%')} color="#34C759" />
                          </TouchableOpacity>
                          <TouchableOpacity 
                            style={styles.imageActionBtn}
                            onPress={() => handleDownloadImage(
                              selectedFormData.capturedPhoto!, 
                              `captured_photo_${selectedFormData.photoTimestamp || Date.now()}.jpg`
                            )}
                          >
                            <Ionicons name="trash" size={hp('2.2%')} color="red" />
                          </TouchableOpacity>
                        </View>
                      </TouchableOpacity>
                    )}
                    {selectedFormData?.images.map((image, index) => (
                      <TouchableOpacity 
                        ...
                        <View style={styles.imageActions}>
                          ...
                          <TouchableOpacity 
                            style={styles.imageActionBtn}
                            onPress={() => handleDownloadImage(image.uri, image.fileName)}
                          >
                            <Ionicons name="trash" size={hp('2.2%')} color="red" />
                          </TouchableOpacity>
                        </View>
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollViewContent}>
        ...
        <Text style={styles.subtitle}>Discipline Design Development Stage List</Text>
...
const styles = StyleSheet.create({
  ...
  modalContent: {
    ...
    padding: wp('4%'),
    width: wp('85%'),
    maxHeight: hp('74%'), justifyContent:"center", alignItems:"center"
  },
  detailModalContent: {
    backgroundColor: '#fff',
    borderRadius: wp('2%'),
